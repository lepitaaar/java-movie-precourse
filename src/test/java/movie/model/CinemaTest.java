...
import movie.model.seat.DefaultSeatGradeRule;
...
class CinemaTest {
    ...
    void 상영관에_영화를_추가할수있다() {
        ...
        try {
            cinema.addMovie(year, month, day,6, movie2);
            cinema.addMovie(year, month, day,10, movie);
            cinema.addMovie(year, month, day,12, movie);
            cinema.addMovie(year, month, day,14, movie2);
            cinema.addMovie(year, month, day,18, movie);
        } catch (Exception e) {
    ...
    void 상영관에_영화를_추가할때_겹치는_시간이_있으면_에러가_발생한다() {
        ...
        try {
            cinema.addMovie(year, month, day, 10, movie); // 10:00 ~ 12:00
        } catch (Exception e) {
            fail("not expected error: " + e.getMessage());
        }
        ...
        Exception exception = assertThrows(Exception.class, () -> {
            cinema.addMovie(year, month, day, 11, new Movie("다른 영화", 2));
        });
        assertEquals("해당 시간에 상영 예정 영화가 존재합니다", exception.getMessage());

        // 2. 정확히 같은 시간에 시작할 때
        Exception exception2 = assertThrows(Exception.class, () -> {
            cinema.addMovie(year, month, day, 10, new Movie("또 다른 영화", 1));
        });
    ...
    void 상영관_폐관시간을_넘겨_영화를_추가할_수_없다() {
        ...
        Exception exception = assertThrows(Exception.class, () -> {
            cinema.addMovie(year, month, day, 18, movie); // 18:00 + 3시간 = 21:00, 폐관시간 20:00 초과
        });
    ...
    void 올바르지_않은_시간에_영화를_추가할_수_없다() {
        ...
        Exception exception = assertThrows(Exception.class, () -> {
            cinema.addMovie(year, month, day, 21, movie);
        });
